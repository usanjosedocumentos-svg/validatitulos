...
if pagina == "Validar titulo":
    ...
    st.info("Registra un diploma y adjuntalo para revision del Back.")
...
**Universidad:** {row['universidad'] or '---'}
**Pais:** {row['pais']}")
    ...
    if CSV_TITULOS.exists():
        dft=pd.read_csv(CSV_TITULOS)
        ap=dft["aplica"].astype(str).str.lower().isin(["true","1","si","yes"]).sum() if "aplica" in dft.columns else 0
        cols[0].metric("Titulos en base",len(dft)); cols[1].metric("Aplican",int(ap)); cols[2].metric("No aplican",len(dft)-int(ap))
    if not df_sol.empty:
