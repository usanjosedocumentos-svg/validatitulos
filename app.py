"""
app.py - ValidaTitulos - Sistema con carga de diplomas
...
def guardar_solicitud(nombre, titulo, universidad, pais, diploma_path, notas=""):
    df = leer_solicitudes()
    nueva = {"id":str(uuid.uuid4())[:8].upper(), "fecha":datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M"),
             "nombre":nombre.strip().upper(), "titulo":titulo.strip().upper(),
             "universidad":universidad.strip().upper(), "pais":pais, "estado":"PENDIENTE",
             "diploma_path":str(diploma_path) if diploma_path else "", "notas":notas}
    df = pd.concat([df, pd.DataFrame([nueva])], ignore_index=True)
    df.to_csv(CSV_SOLICITUDES, index=False)
    return nueva["id"]

def actualizar_estado(sol_id, estado):
    df = leer_solicitudes()
    df.loc[df["id"] == sol_id, "estado"] = estado
    df.to_csv(CSV_SOLICITUDES, index=False)
...
with st.sidebar:
    ...
    n = len(df_sol[df_sol["estado"]=="PENDIENTE"]) if not df_sol.empty else 0
    if n: st.markdown(f"<div class='btn-pendiente'>PENDIENTES BACK: {n}</div>", unsafe_allow_html=True)
    ...
    if CSV_TITULOS.exists():
        ...
        st.metric("Registros totales", len(dft))
        st.metric("Aplican", int(ap))
    consultas = 0
...
if pagina == "Validar titulo":
    ...
    df_sol = leer_solicitudes()
    np2 = len(df_sol[df_sol["estado"]=="PENDIENTE"]) if not df_sol.empty else 0
    if np2: st.warning(f"El Back tiene {np2} solicitud(es) pendiente(s) por aprobar.")
            ...
            with st.expander(f"#{row['id']} - {row['titulo']} - {row['nombre']} - {row['fecha']}"):
                ...
                with cf:
                    ...
                    with st.form(f"form_back_{row['id']}"):
                        ...
                        PB = PAISES
                        bp = st.selectbox("Pais", PB, index=PB.index(row["pais"]) if row["pais"] in PB else 0)
                    ...
                    if bs and bt.strip():
                        ...
                        actualizar_estado(row["id"], "APROBADA" if ba=="Si" else "RECHAZADA")
    ...
    if archivo:
        ...
        if falt: st.error(f"Faltan: {falt}")
            ...
            if CSV_TITULOS.exists():
                dfb = pd.read_csv(CSV_TITULOS, dtype=str).fillna("")
                dfb["nombre_titulo"] = dfb["nombre_titulo"].str.upper().str.strip()
    ...
    if CSV_TITULOS.exists():
        dft = pd.read_csv(CSV_TITULOS)
        ap = dft["aplica"].astype(str).str.lower().isin(["true","1","si","yes"]).sum() if "aplica" in dft.columns else 0
        cols[0].metric("Titulos en base", len(dft))
        cols[1].metric("Aplican", int(ap))
        cols[2].metric("No aplican", len(dft)-int(ap))
    if not df_sol.empty:
    ...
    if CSV_DECISIONES.exists():
        ...
        if not dfd.empty and "nivel_confirmado" in dfd.columns:
            c = dfd["nivel_confirmado"].value_counts().reset_index()
            c.columns = ["Nivel","Cantidad"]
            st.dataframe(c, use_container_width=True, hide_index=True)
    else: st.info("Sin datos de decisiones Back aun.")
